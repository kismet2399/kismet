...
//    类别是英文的，如auto，yule. 对string类型的label做Double类型的编码
    ...
    df_tfidf_lab.show()
    df_tfidf_lab.select("lable","indexed").distinct()
...
//    模型预测
    ...
    pred.select("rawPred","prob").show(2,false)
...
//    hdfs path
    ...
    nbModel.pi
    nbModel.theta
    nbModel.getModelType
    nbModel.getLabelCol
    nbModel.explainParam(nbModel.labelCol)
  }
