...
object SparkHbase {
  ...
  def main(args: Array[String]): Unit = {
    //    HBase zookeeper
    val ZOOKEEPER_QUORUM = "192.168.181.10,192.168.181.11,192.168.181.12"
    val spark = SparkSession.builder().appName("spark to Hbase")
      .enableHiveSupport().getOrCreate()

    val rdd = spark.sql("select order_id,user_id,order_dow from orders limit 300")
      .rdd
    ...
    rdd.foreach(row =>System.out.println(row(0).asInstanceOf[String] + "=========" + row(1).asInstanceOf[String] + "=========" + row(2).asInstanceOf[String] + "========="))
    logger.error(list)
    logger.error("Logger=========================================================================")
